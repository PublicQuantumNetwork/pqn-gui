...
if (!isset($_SESSION["pull_question_number_id"])) {
	$_SESSION["pull_question_number_id"] = 13;
}

if ($_SESSION["pull_question_number_id"] > 13) {
	$_SESSION["pull_question_number_id"] = 13;
}

if (!isset($_SESSION["pull_question_count"])) {
	$_SESSION["pull_question_count"] = 0;
}
...
$half_question_count = ($question_count/2);

//do not set to $total_question_count > $question_count - will error!!!
if ($_SESSION["pull_question_count"] >= 4) {
	...
	//echo $sql . "<BR>";
	//echo $protocol_id . "<BR>";
	//echo $_SESSION["pull_question_number_id"] . "<BR>";
	$st = $conn->prepare($sql);
	$st->bindValue(':protocol_id', $protocol_id);
	//$st->bindValue(':name_id', $setid);
	...
	if ($st->rowCount() > 0){
		...
		$_SESSION["pull_question_count"] = $_SESSION["pull_question_count"] + 1;
	}
}
...
//echo $_SESSION["pull_question_count"] . "<BR>";
//echo $total_question_count . "<BR>";
//echo $half_question_count . "<BR>";
?>
