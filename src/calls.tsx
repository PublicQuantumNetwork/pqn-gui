...
export async function submitFortune() {
  const response = await fetch(
    `http://${process.env.NEXT_PUBLIC_API_ADDRESS}/rng/fortune?timetagger_address=${process.env.NEXT_PUBLIC_TIMETAGGER_ADDRESS}&integration_time_s=0.1&fortune_size=6&channels=1`,
    {
      method: 'GET',
    }
  );
...
export async function fetchRotatorAngle() {
  try {
    const response = await fetch(
      `http://${process.env.NEXT_PUBLIC_API_ADDRESS}/serial/`
    );
...
export async function resetBackendState() {
  const response = await fetch(
    `http://${process.env.NEXT_PUBLIC_API_ADDRESS}/coordination/reset_coordination_state`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export async function fetchQuestionOrder() {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const response = await fetch(
        `http://${process.env.NEXT_PUBLIC_API_ADDRESS}/qkd/question_order`,
        {
          method: 'GET',
        }
      );
      ...
      if (Array.isArray(questionOrder) && questionOrder.length === 0) {
        if (attempt < maxRetries) {
          console.log(
            `Question order is empty, retrying... (attempt ${attempt}/${maxRetries})`
          );
          ...
          return {
            success: false,
            error: 'Question order not available after 5 attempts',
            questionOrder: [],
          };
        }
      }
  ...
  return {
    success: false,
    error: 'Failed to fetch question order',
    questionOrder: [],
  };
}
